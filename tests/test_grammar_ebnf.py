...
def test_ebnf_bracket_1():
    ...
    shared.check_grammar(
        grammar,
        ['', '0', '1', '00', '01', '10', '11', '1110', '3', '23', '223', '4', '45 5', '45 55 5'],
        ['45']
    )
...
def test_ebnf_quantifier_bracket_combination():
    ...
    shared.check_grammar(
        grammar,
        ['1', 'a1', 'b1', '', '3', '23', '223', '2223', '2323', '223232223', '233',
         '48', '45 58', '45 55 58', '484848', '4845 55 5845 5848'],
        ['aa1', '45 5', '485 5'],
    )


def test_ebnf_warning_1(caplog):
  ...
  B as nonterminal <B> and terminal "B"'''
    shared.emits_warning(lambda: al.Grammar(ebnf_text=ebnf_text), caplog, expected_message)


def test_ebnf_warning_2(caplog):
...
def test_ebnf_warning_3(caplog):
    # A grammar where sets overlap and a rule is redundant
    ebnf_text = """
    S = A | "1" "2" | B "3" $§ €
    A = B "4" | "5" | B
    B = "6" "7" | A B
    ...
    """
    # Expected warnings
    expected_message1 = '''Problematic grammar specification: The sets of nonterminal and terminal symbols are not disjoint, as required by the mathematical definition of a grammar. This package can deal with it, but in general it is not recommended. Following symbols appear in both sets:
  A as nonterminal <A> and terminal "A"'''
    expected_message2 = """Problematic grammar specification: Some production rules are redundant. This package can deal with it, but in general it is not recommended. In particular, it introduces bias in random string generation. Following rules are contained more than one time:
  <A> ::= <B> 4"""
    shared.emits_warning(
        lambda: al.Grammar(ebnf_text=ebnf_text), caplog, [expected_message1, expected_message2])
...
def test_ebnf_unusual_symbols():
    ...
    grammar = al.Grammar(
        ebnf_text=ebnf_text,
        defining_symbol='1314', rule_separator_symbol='`~',
        start_nonterminal_symbol='ZZ', end_nonterminal_symbol='%',
        start_terminal_symbol='""', end_terminal_symbol='/',
