...
def generate_language(grammar, max_steps=None, sort_order=None, verbose=None,
                      return_details=None):
    """Generate the language (set of all strings) described by a grammar.

    Notes
    -----
    This algorithm uses the production rules in a reverse fashion to generate
    the languages of each non-terminal, starting from rules that contain
    only terminals on their right-hand side. At the end, the lanugage of the
    start symbol can be returned, which is the language of the grammar.
    Since more information was generated, it is also possible to return
    each language of each non-terminal.
    ...
    if verbose:
        ...
        if max_steps > 1:
            ...
            print('=' * len(text))
            print()
            print('Production rules with at least one nonterminal in the right-hand side:')
            for lhs, rhs, nonterminal, nonterminal_pos in nonterminal_rules:
                rhs_repr = ''.join('<{}>'.format(sym) if i in nonterminal_pos else sym
    ...
    for num_expansion in range(2, max_steps+1):
        ...
        for lhs, rhs, nonterminals, nonterminal_positions in nonterminal_rules:
            ...
            for nonterminal_choices_list in wanted_selections:
                ...
                for comb_of_known_strings in _cartesian_product(*string_lists_per_nt):
                    new_string = rhs[:]
                    for pos, known_string in zip(nonterminal_positions, comb_of_known_strings):
                        new_string[pos] = known_string
                    new_string = _terminal_symbol_seq_to_str(new_string)
                    new_strings[lhs][new_string] = None  # add the key, value is irrelevant
        ...
        for nonterminal in grammar.nonterminal_symbols:
            ...
            strings_recent[nt_str] = {string: None for string in new_strings[nt_str]
        ...
        if warn_if_stopped:
            _logging.warn_language_gen_stopped(max_steps)
        for nonterminal in grammar.nonterminal_symbols:
            nt_str = nonterminal.text
            strings_all[nt_str].update(strings_recent[nt_str])
    ...
    if return_details:
        ...
        return {nonterminal: sort_func(strings) for nonterminal, strings in strings_all.items()}
    else:
