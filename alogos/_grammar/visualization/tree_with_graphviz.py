...
def create_graphviz_tree(tree, show_node_indices=None,
    ...
    shapes = [
        'Mcircle', 'Mdiamond', 'Msquare', 'assembly', 'box', 'box3d', 'cds', 'circle',
        'component', 'cylinder', 'diamond', 'doublecircle', 'doubleoctagon', 'egg',
        'ellipse', 'fivepoverhang', 'folder', 'hexagon', 'house', 'insulator', 'invhouse',
        'invtrapezium', 'invtriangle', 'larrow', 'lpromoter', 'none', 'note', 'noverhang',
        'octagon', 'oval', 'parallelogram', 'pentagon', 'plain', 'plaintext', 'point',
        'polygon', 'primersite', 'promoter', 'proteasesite', 'proteinstab', 'rarrow',
        'rect', 'rectangle', 'restrictionsite', 'ribosite', 'rnastab', 'rpromoter',
        'septagon', 'signature', 'square', 'star', 'tab', 'terminator', 'threepoverhang',
        'trapezium', 'triangle', 'tripleoctagon', 'underline', 'utr', None,
    ]

    show_node_indices = _ap.bool_arg(
        'show_node_indices', show_node_indices, default=False)
    layout_engine = _ap.str_arg(
        'layout_engine', layout_engine, default='dot',
        vals=['circo', 'dot', 'fdp', 'neato', 'osage', 'patchwork', 'sfdp', 'twopi', None])
    fontname = _ap.str_arg(
        'fontname', fontname, default='Mono')
    fontsize = _ap.int_arg(
        'fontsize', fontsize, default=12)
    shape_nt = _ap.str_arg(
        'shape_nt', shape_nt, vals=shapes, default='box')
    shape_unexpanded_nt = _ap.str_arg(
        'shape_unexpanded_nt', shape_unexpanded_nt, vals=shapes, default='box')
    shape_t = _ap.str_arg(
        'shape_t', shape_t, vals=shapes, default='ellipse')
    fontcolor_nt = _ap.str_arg(
        'fontcolor_nt', fontcolor_nt, default='black')
    fontcolor_unexpanded_nt = _ap.str_arg(
        'fontcolor_unexpanded_nt', fontcolor_unexpanded_nt, default='white')
    fontcolor_t = _ap.str_arg(
        'fontcolor_t', fontcolor_t, default='white')
    fillcolor_nt = _ap.str_arg(
        'fillcolor_nt', fillcolor_nt, default='white')
    fillcolor_unexpanded_nt = _ap.str_arg(
        'fillcolor_unexpanded_nt', fillcolor_unexpanded_nt, default='#b81118')
    fillcolor_t = _ap.str_arg(
        'fillcolor_t', fillcolor_t, default='#00864b')
...
class DerivationTreeFigure:
    ...
    def display(self, inline=False):
        ...
        if inline:
            ...
            display(SVG(self.fig._repr_svg_()))
        else:
