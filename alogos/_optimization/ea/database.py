...
class Database:
    ...
    def __str__(self):
        ...
        msg.append('│ Number of changes ............ {}{}'.format(num_changes, _NEWLINE))
        msg.append('│ Number of generations ........ {}{}'.format(num_generations, _NEWLINE))
        msg.append('│ Number of individuals ........ {}{}'.format(num_individuals, _NEWLINE))
        msg.append('│ Number of unique genotypes ... {}{}'.format(num_genotypes, _NEWLINE))
        msg.append('│ Number of unique phenotypes .. {}{}'.format(num_phenotypes, _NEWLINE))
        msg.append('│ Number of unique fitnesses ... {}{}'.format(num_fitness, _NEWLINE))
        msg.append('│ Minimum population size ...... {}{}'.format(pop_size_min, _NEWLINE))
        msg.append('│ Maximum population size ...... {}{}'.format(pop_size_max, _NEWLINE))
        if isinstance(min_fitness, float):
            msg.append('│ Minimum fitness .............. {:.6f}{}'.format(min_fitness, _NEWLINE))
            msg.append('│   shared by {} individuals, {} genotypes, {} phenotypes{}'.format(
                num_min_ind, num_min_genotypes, num_min_phenotypes, _NEWLINE))
            msg.append('│ Maximum fitness .............. {:.6f}{}'.format(max_fitness, _NEWLINE))
            msg.append('│   shared by {} individuals, {} genotypes, {} phenotypes{}'.format(
                num_max_ind, num_max_genotypes, num_max_phenotypes, _NEWLINE))
        else:
            msg.append('│ Minimum fitness .............. {}{}'.format(min_fitness, _NEWLINE))
            msg.append('│ Maximum fitness .............. {}{}'.format(max_fitness, _NEWLINE))

        # First individual with min fitness
        if min_individuals:
            msg.append('│{}'.format(_NEWLINE))
            first_min_individual = min_individuals[0]
            msg.append('│ First individual with minimum fitness {:.6f}{}'.format(
        ...
        if num_min_phenotypes > 1:
            ...
            msg.append('│ First {} of {} phenotypes with minimum fitness {:.6f}{}'.format(
        ...
        if max_individuals:
            ...
            msg.append('│ First individual with maximum fitness {:.6f}{}'.format(
        ...
        if num_max_phenotypes > 1:
            ...
            if num_max_phenotypes <= max_shown_phenotypes:
                ...
                num_shown_phenotypes = max_shown_phenotypes
            msg.append('│ First {} of {} phenotypes with maximum fitness {:.6f}{}'.format(
    ...
    def _try_creating_views(self):
        """Create views in the database, which join some tables for simpler access to data."""
        try:
    ...
    def _store_population(self, label, population, generation):
        ...
        Notes
        -----
        - Each individual requires multiple INSERT statements. If they were commited
          as separate transactions, it would take a lot of time. Instead, the data
          of all individuals can be collected and stored in a single transaction.

        - The details attribute of each individual can contain an evaluation key
          that refers to user-defined data returned by the objective function.
          It is attempted to be JSON serialized and in case it fails the data
          of this attribute is not stored and an empty dictionary instead without
          warning.
    ...
    def _load_population(self, generation, with_parent_ids=True):
        ...
        The information in from the database is converted to suitable Python objects
        on the level of individual attributes (e.g. fitness is float),
        individual objects (type depends on system)
        and population (type depends on system).

        Raises
        ------
        DatabaseError
            If the loaded population is empty because the database does not
            contain the user-provided generation.
    ...
    def _load_database_subset(self, first_gen, last_gen):
        ...
        genotype_phenotype_mapping = self._dbms.execute_query(query, (first_gen, last_gen))
        ...
        phenotype_fitness_mapping = self._dbms.execute_query(query, (first_gen, last_gen))
    ...
    def _load_database_full(self):
        ...
        return self._load_database_subset(self.generation_first(), self.generation_last())

    # File I/O
    def export_sql(self, filepath, ext='sqlite3'):
        """Export the evolutionary search by storing the current database to an SQL file.

        All tables and views are exported to a single SQL file that adheres to
        `SQLite version 3 <https://docs.python.org/3/library/sqlite3.html>`__.
        It can be used for a later import in order to continue and analyze a run,
        or it may be opened with an external tools such as
        ...
        filepath : str
            ...
            - It is ensured to be a filepath that does not exist yet by adding a numerical suffix.
              ...
              Example: If ``some_file.sqlite3`` exists, it uses ``some_file_1.sqlite3`` or
              if that also exists then ``some_file_2.sqlite3`` and so on.
        ext : str
        ...
        if ext is None:
            ...
            filepath_used = _utilities.operating_system.ensure_file_extension(filepath, ext)
    ...
    def export_csv(self, filepath, ext='csv'):
        ...
        Only the main view, which gathers information from all individual tables,
        is exported to a single CSV file. As such it provides all information
        stored in the database in a redundant manner. Currently it can not be used
        for a later import, but it can be opened with external tools that can read
        CSV files, such as
    ...
    def import_sql(self, filepath, generation_range=None):
        ...
        Caution: The method :meth:`reset` is called, so that the current state and
        database are dropped and can be replaced in a clean fashion by new data
        from the SQL file.
        ...
        generation_range : TODO

        Examples
        --------
        Using ``first_generation=0`` and ``last_generation=2`` loads the
        first three generations of a previous run. The last generation is
        reconstructed as population in memory, so that the search can be
        continued from this point. If :meth:`run_step()` is called,
        ...
        if first_generation < first_gen_in_db or first_generation > last_gen_in_db:
            _logging.warn_database_import_first(first_gen_in_db, first_generation)
            first_generation = first_gen_in_db
        if last_generation > last_gen_in_db or last_generation < first_generation:
            _logging.warn_database_import_last(last_gen_in_db, last_generation)
    ...
    def import_sql_evaluations(self, filepath, verbose=False):
        ...
        is computationally demanding and prevention of some recalculations
        may speed up the search significantly.
        ...
        if len(phe_fit_evaluations) == 0:
            _logging.warn_import_database_empty(filepath)
        ...
        if verbose:
            ...
            message = (
                ...
                'external database at {}.').format(num_eval, filepath)
            print(message)
    ...
    def num_individuals(self, generation_range=None, only_main=False):
        ...
        if generation_range is None:
            ...
            query = (
                'SELECT COUNT(individual_id) FROM search '
                'WHERE label={} AND generation BETWEEN ? AND ?;').format(label)
    ...
    def num_fitnesses(self, generation_range=None, only_main=False):
        ...
        NaN values are not counted. These values appear in individuals that
        were not evaluated, e.g. those generated by crossover but then
        modified by mutation before being evaluated and selected.
    ...
    def num_phe_to_fit_evaluations(self, only_unique=True):
        ...
        which depends on the parametrization (cache and/or database lookups).
        ...
        query = (
            ...
            'ON t2.phenotype=t1.phenotype;').format(column)
        result = self._dbms.execute_query(query)
        return result[0][0]
    ...
    def _process_generation_range(self, generation_range):
        """Process the user-provided generation range so it can be used safely in an SQL query."""
        _ap.check_arg('generation_range', generation_range, (type(None), int, _Iterable))
    ...
    def individuals_with_given_fitness(self, fitness, generation_range=None, only_main=False):
        ...
        if generation_range is None:
            query = (
                'SELECT * FROM full_search '
                'WHERE label={} AND fitness=?;').format(label)
            result = self._dbms.execute_query(query, (fitness,))
        else:
            query = (
                'SELECT * FROM full_search '
                'WHERE label={} AND generation BETWEEN ? AND ? AND fitness=?;').format(label)
    ...
    def individuals_with_min_fitness(self, generation_range=None, only_main=False):
        ...
        try:
            ...
            individuals = self.individuals_with_given_fitness(value, generation_range, only_main)
        except _exceptions.DatabaseError:
            individuals = []
        return individuals
    ...
    def individuals_with_low_fitness(self, n=10, generation_range=None, only_main=False):
    ...
    def individuals_with_high_fitness(self, n=10, generation_range=None, only_main=False):
    ...
    def genotypes_with_given_fitness(self, fitness, generation_range=None, only_main=False):
    ...
    def genotypes_with_min_fitness(self, generation_range=None, only_main=False):
        ...
        try:
            value = self.fitness_min(generation_range, only_main)
            genotypes = self.genotypes_with_given_fitness(value, generation_range, only_main)
        except _exceptions.DatabaseError:
            genotypes = []
        return genotypes
    ...
    def phenotypes_with_given_fitness(self, fitness, generation_range=None, only_main=False):
    ...
    def phenotypes_with_min_fitness(self, generation_range=None, only_main=False):
        ...
        try:
            value = self.fitness_min(generation_range, only_main)
            phenotypes = self.phenotypes_with_given_fitness(value, generation_range, only_main)
        except _exceptions.DatabaseError:
            phenotypes = []
        return phenotypes
    ...
    def details_with_given_fitness(self, fitness, generation_range=None, only_main=False):
    ...
    def details_with_min_fitness(self, generation_range=None, only_main=False):
        ...
        try:
            value = self.fitness_min(generation_range, only_main)
            details = self.details_with_given_fitness(value, generation_range, only_main)
        except _exceptions.DatabaseError:
            details = []
        return details
    ...
    def phe_to_fit_evaluations(self, num_evaluations=None, with_details=False):
        ...
        if num_evaluations is None:
            query = (
                ...
                'LEFT JOIN phenotype_fitness_mapping AS t2 '
                'ON t2.phenotype=t1.phenotype;').format(tables)
            result = self._dbms.execute_query(query)
        else:
            query = (
                'SELECT {} FROM genotype_phenotype_mapping AS t1 '
        ...
        def create_list():
            ...
            for i in range(len(data)):
                ...
                data[i] = [
                    ...
                    self._deserializer.details(row[7]),        # details [None, JSON object, str]
                    # derived
            ...
            def assign_ranks(row_idx, fitnesses):
                ...
                if fitnesses:
                    rank_idx = list(range(len(fitnesses), 0, -1))  # consider entries -n to -1
                    rank_idx_fit = list(zip(rank_idx, fitnesses))
                    rank_idx_fit.sort(key=lambda x: x[1])  # sort by fitness to distribute ranks
                    for rank, (rank_idx, fit) in enumerate(rank_idx_fit):
                        data[row_idx-rank_idx][10] = rank  # assign ranks to entries -n to -1
    ...
    def to_columns(self):
        ...
        columns = (
            'individual_id', 'parent_ids', 'generation', 'label', 'genotype',
            'phenotype', 'fitness', 'details', 'genotype_length',
            'phenotype_length', 'rank')
        return columns

    def to_dataframe(self, generation_range=None, only_main=False):
        """Convert the database entries to a Pandas DataFrame and add some derived information.

        Parameters
        ----------
        only_main_populations : bool
            From the complete dataframe, only main populations are kept and intermediate ones
            (selected parents, crossed-over or mutated populations) are filtered out. Note
            that the crossed-over population is not evaluated if a mutation operator is
            provided, so that they have no associated phenotype and fitness values.
    ...
    def to_network(self, generation_range=None, only_main=False):
        ...
        # Edges
        columns = (
            'parent_id', 'individual_id', 'individual_x', 'parent_x',
            'individual_y', 'parent_y', 'individual_z', 'parent_z', 'individual_label',
            'style', 'color', 'width', 'individual_generation')
        for row in edges:
            graph.add_edge(row[0], row[1], **{key: val for key, val in zip(columns[1:], row[1:])})
        return graph
    ...
    def _to_graph(self, generation_range, only_main):
        ...
        # Optional filtering
        nodes, edges = self._filter_graph(data, nodes, edges, generation_range, only_main)
        return nodes, edges
    ...
    def _filter_graph(self, data, nodes, edges, generation_range=None, only_main=False):
        ...
        if filter_gen and only_main:
            ...
            def keep_node(row, used_ind_ids):
                gn = row[2]
                if (gn == first_gen and row[3] == 'main') or (gn > first_gen and gn <= last_gen):
                    used_ind_ids.add(row[0])
                    return True
                return False
        else:
    ...
    def plot_genealogy(self, backend='vis', generation_range=None, only_main=False,
                       **kwargs):
        """It shows the relations between all individuals created throughout the run."""
        # Argument processing
        backend = _ap.str_arg(
            'backend', backend, vals=('d3', 'vis', 'three'))
        ...
        # Plot
        fig = _plots.genealogy(data, graph, backend, **kwargs)
        return fig
    ...
    def _lookup_or_calc(self, key, calc_func, *calc_args):
        ...
        try:
            ...
            result = _copy.copy(result)  # prevent shallow modification of original, deep too slow
...
class Deserializer():
    ...
    def genotype(self, data):
        """Reconstruct a Genotype object from raw data of an entry in the database.

        The type of the Genotype object depends on the
        grammar-based genetic programming system being used.

        """
        # Note: String representation would work for most use cases too, but not for len()
    ...
    def gt_phe_map(self, data):
        return [(self.genotype(row[0]),
                 self.phenotype(row[1])) for row in data]

    def phe_fit_map(self, data):
        return [(self.phenotype(row[0]),
                 self.fitness(row[1])) for row in data]

    def phe_fit_det_map(self, data):
        return [(self.phenotype(row[0]),
                 self.fitness(row[1]),
                 self.details(row[2])) for row in data]

    def individual(self, data, without_parent_ids=False):
        """Reconstruct an Individual object from raw data of a row in the database.

        The type of the Individual object depends on the
        grammar-based genetic programming system being used.
    ...
    def population(self, data):
        """Reconstruct a Population object from raw data of a row in the database.

        The type of the Population object depends on the
        grammar-based genetic programming system being used.
