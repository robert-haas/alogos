...
def read_bnf(grammar, bnf_text, verbose, *symbols):
    ...
    def_symbol, rule_separator_symbol, start_nt, end_nt, start_t1, end_t1, start_t2, end_t2 = \
    ...
    if start_nt and end_nt:
        pattern_nt = r'{}{}{}'.format(esc_start_nt, _ENCLOSED_NONTERMINAL_PATTERN, esc_end_nt)
    else:
    ...
    if pattern_t1 and pattern_t2:
        ...
        re_t = None
    # Right-hand sides
    pattern_nonempty = [pat for pat in [pattern_nt, pattern_t1, pattern_t2] if pat is not None]
...
def read_ebnf(grammar, ebnf_text, verbose, *symbols):
    ...
    re_lhs_rhs = _re.compile(pattern_lhs_rhs)
    ebnf_text = _NEWLINE + ebnf_text  # newline to guarantee recognition of the first lhs
    ...
    def process_a_nt_special_mix(rhs_text):
        seq = [process_a_nonterminal(text, crop=True) if text_is_a_nonterminal(text) else text
    ...
    def find_an_innermost_bracket_pair(txt_list):
        ...
        for idx, text in enumerate(txt_list):
            if isinstance(text, str):
                if text in [start_group_symbol, start_option_symbol, start_repeat_symbol]:
                    last_opening_symbol = text
                    last_opening_symbol_idx = idx
                elif text == end_group_symbol and last_opening_symbol == start_group_symbol:
                    found_bracket = ('group', last_opening_symbol_idx, idx)
                    break
                elif text == end_option_symbol and last_opening_symbol == start_option_symbol:
                    found_bracket = ('option', last_opening_symbol_idx, idx)
                    break
                elif text == end_repeat_symbol and last_opening_symbol == start_repeat_symbol:
    ...
    def create_nt_with_derived_name(text, suffix):
        cnt = 0
        while True:
            new_text = '{text}{suffix}{count}'.format(text=text, suffix=suffix, count=cnt)
    ...
    def remove_brackets(rule):
        ...
        while True:
            ...
            sym_nt_new = create_nt_with_derived_name(sym_lhs_nt.text, suffix=_SHARED_SUFFIX)
            left, sym_list_inner, right = cut_out_exclusive(sym_list_rhs, start_idx, end_idx)
    ...
    while i < len(txt_list_lhs_rhs):
        ...
        # Parse rhs: Get several productions, each consisting of arbitrary symbols
        t_vs_others_seq = [process_a_terminal(text) if text_is_a_terminal(text) else text
...
def write_bnf(grammar, rules_on_separate_lines, *symbols):
    ...
    # Argument processing
    rules_on_separate_lines = _ap.bool_arg('rules_on_separate_lines', rules_on_separate_lines)
    def_symbol, rule_separator_symbol, start_nt, end_nt, start_t1, end_t1, start_t2, end_t2 = \
        _check_symbols_validity(*symbols)
...
def _check_symbols_validity(defining_symbol, rule_separator_symbol,
    ...
    return def_symbol, rule_separator_symbol, start_nt, end_nt, start_t1, end_t1, start_t2, end_t2


def _check_grammar_validity(grammar):
    """Check some criteria that a proper context-free grammar needs to fulfill.
    ...
    if intersection:
        _logging.warn_symbol_set_overlap(intersection)
    ...
    if len(lhs_nonterminals) != (len(rhs_nonterminals) - 1):
        missing_nonterminals = ['<{}>'.format(nt) for nt in rhs_nonterminals
                                if nt not in lhs_nonterminals]
    ...
    for lhs, rhs_vals in grammar.production_rules.items():
        seen = set()
        for rhs in rhs_vals:
            rhs_text = ' '.join('<{}>'.format(sym.text)
                                if isinstance(sym, _data_structures.NonterminalSymbol)
                                else sym.text for sym in rhs)
            if rhs_text in seen:
                text = '  <{}> ::= {}'.format(str(lhs), rhs_text)
                repeated_productions[text] = None
            seen.add(rhs_text)
    if repeated_productions:
        _logging.warn_repeated_productions(repeated_productions)
