...
class RobustCallable:
    """Phenotype-to-fitness function made robust against failure and invalid return values.

    It provides following guarantees:
    - Result is a tuple with two entries: (fitness, details)
    - fitness is ensured to be of type float, allowing +Inf and -Inf, but excluding NaN
    - details can be any user-chosen type

    Notes
    -----
    - Simply wrapping the function into another function would cause pickling errors.
    - If the robust fitness function cannot be pickled, most parallel and distributed
      evaluation solutions cannot be applied since they rely on some pickling function.
        ...
        # 2) Valid phenotype
            ...
            try:
                ...
                if not isinstance(fit, _Number):
                    message = 'Returned fitness value is not a number: {}'.format(fit)  # TODO: move into excp
                    raise ValueError(message)

                if fit != fit:
                    message = (
                        'Returned fitness value is NaN. It is replaced by the '  # TODO: improve text
