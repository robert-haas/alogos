...
def is_recursive(grammar):
    ...
    A nonterminal is considered to be recursive if it can produce itself,
    i.e. if starting from it some derivation can be constructed where the
    nonterminal appears again.

    Examples:

    - ``S => SS => 1S => 12`` means that ``S`` is trivially recursive, because
      there is a rule that replaces ``S`` directly with a sequence of symbols
      that contains ``S`` again.

    - ``S => A => BB => 5B => 5S => 51`` means ``S`` is non-trivially recursive,
      because ``S`` can be replaced indirectly (via other nonterminals and their rules)
      with a sequence of symbols that contains ``S`` again.
    ...
    def can_nt_produce_itself(nt):
        """Check if a nonterminal can produce itself by following each derived nonterminal once"""
        observed = set([nt])
        stack = [nt]
        while stack:
            symbol = stack.pop()
            for rhs in grammar.production_rules[symbol]:
                for new_symbol in rhs:
                    if isinstance(new_symbol, _grammar.data_structures.NonterminalSymbol):
                        if new_symbol == nt:
                            return True
    ...
    is_nt_recursive = {nt: can_nt_produce_itself(nt) for nt in grammar.nonterminal_symbols}
...
def min_depths_to_terminals(grammar):
    ...
    This information is used during a derivation in order to remain within
    some maximum depth limit.

    Algorithms that utilize it:

    - Initialization of an inidividual with "grow", "pi grow" and "full" methods that
      are part of ramped half-and-half (rhh) population initialization.
    ...
    min_depths = dict()
    for lhs, rhs_multiple in grammar.production_rules.items():
        min_depths[lhs] = [1 if _contains_only_terminals(rhs) else _MAX_INT
                           for rhs in rhs_multiple]
...
def min_expansions_to_terminals(grammar):
    ...
    min_expansions = dict()
    for lhs, rhs_multiple in grammar.production_rules.items():
        min_expansions[lhs] = [0 if _contains_only_terminals(rhs) else _MAX_INT
                               for rhs in rhs_multiple]
...
def min_nodes_to_terminals(grammar):
    ...
    min_nodes = dict()
    for lhs, rhs_multiple in grammar.production_rules.items():
        min_nodes[lhs] = [len(rhs) if _contains_only_terminals(rhs) else _MAX_INT
                          for rhs in rhs_multiple]
